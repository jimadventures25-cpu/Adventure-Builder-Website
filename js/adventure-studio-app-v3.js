...
function videoState(section){
 const left=$(".ab-video-left",section), main=$(".ab-video-main",section), groups=left?$$(":scope > .ab-video-group",left):[];
 const upload=$(".ab-video-upload",left), exp=$("#ab-video-export",section);
 return {
  section,canvas:main,upload,top:[exp].filter(Boolean),
  defs:[["▤","Clips","clips"],["▣","Format","format"],["↔","Timeline","timeline"],["✦","Effects","effects"],["▦","Templates","templates"]],
  tools:{
   clips:{title:"Clips",nodes:[groups[0]]},
   format:{title:"Format & title",nodes:[groups[1]]},
   timeline:{title:"Timeline & transitions",nodes:[groups[2]]},
   effects:{title:"Effects",nodes:[groups[3]]},
   templates:{title:"Templates",nodes:[groups[4],groups[5]]}
  },initial:"clips"
 };
}
...
function populateBar(kind){
 ...
 const s=state[kind];
 [s.upload,...s.top].filter(Boolean).forEach(n=>{n.classList.add("ab-studio-app__transported");bar.insertBefore(n,spacer.nextSibling)});
}
